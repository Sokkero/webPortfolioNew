...
function ContactMeBlockWrapper() {
	...
	const sendEmail = (e) => {
		...
		emailjs.sendForm(ContactMeContent.emailService.serviceId, ContactMeContent.emailService.templateId, formRef.current, {
			publicKey: ContactMeContent.emailService.publicKey,
			blockHeadless: ContactMeContent.emailService.blockHeadless,
			limitRate: {throttle: ContactMeContent.emailService.requestThrottle}
		})
			.then(
				() => {
